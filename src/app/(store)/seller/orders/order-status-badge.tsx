...
export function OrderStatusBadge({ orderId, status }: { orderId: string; status: string }) {
  ...
  const update = async (value: string | null) => {
    ...
    try {
      ...
      toast.success(`${t.seller.orderMarked} ${t.orders.statuses[value as keyof typeof t.orders.statuses] ?? value}`);
      router.refresh();
    } catch {
      setCurrent(prev);
  ...
  return (
    <Select value={current} onValueChange={update}>
      <SelectTrigger className={`h-7 text-xs w-32 font-semibold border ${colors[current]} cursor-pointer`}>
        <SelectValue />
      </SelectTrigger>
      <SelectContent>
        {statuses.map((s) => (
